...
function MintBtn(props: {
  ...
  console.debug('usePrepareContractWrite', {
    config,
    prepareError,
    isPrepareError
  })
  ...
  console.debug('useContractWrite', {
    freeMint,
    isWriteError,
    writeError,
    mintResult
  ...
  return (
    <>
      <button
        ...
        className="btn btn-accent h-[45px] line-height-[45px]"
        style={{ backgroundColor: mintBtnDisabled ? 'gray' : '' }}
        onClick={() => {
          console.debug('freeMint Clicked')
          freeMint?.()
        }}
      >
      ...
      {isSuccess && (
        <Dialog
          ...
          footer={
            <div
              className="btn btn-secondary text-center color-[white]"
              key="ok"
              onClick={() => {
        ...
        >
          <p>
            Successfully minted! Check it on{' '}
